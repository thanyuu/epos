    ...
    static class SimplePathStrategy implements PathStrategy {
        ...
        public Uri getUriForFile(File file) {
            try {
                ...
                if (rootPath.endsWith(HttpUtils.PATHS_SEPARATOR)) {
                    path = path.substring(rootPath.length());
                } else {
                    path = path.substring(rootPath.length() + 1);
                }
                return new Builder().scheme("content").authority(this.mAuthority).encodedPath(Uri.encode((String) mostSpecific.getKey()) + '/' + Uri.encode(path, HttpUtils.PATHS_SEPARATOR)).build();
            } catch (IOException e) {
