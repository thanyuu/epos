package com.umeng.analytics.game;

import android.content.Context;
import com.umeng.analytics.MobclickAgent;
import com.umeng.analytics.social.C0314e;
import com.umeng.analytics.social.UMPlatformData;
import com.umeng.analytics.social.UMSocialService;
import p000u.aly.bl;

public class UMGameAgent extends MobclickAgent {
    private static final String f606a = "Input string is null or empty";
    private static final String f607b = "Input string must be less than 64 chars";
    private static final String f608c = "Input value type is negative";
    private static final String f609d = "The int value for 'Pay Channels' ranges between 1 ~ 99 ";
    private static final C0605c f610e = new C0605c();
    private static Context f611f;

    public static void init(Context context) {
        f610e.m777a(context);
        f611f = context.getApplicationContext();
    }

    public static void setTraceSleepTime(boolean z) {
        f610e.m781a(z);
    }

    public static void setPlayerLevel(int i) {
        f610e.m778a(String.valueOf(i));
    }

    public static void startLevel(String str) {
        if (m765a(str)) {
            bl.m340e(f606a);
        } else if (str.length() > 64) {
            bl.m340e(f607b);
        } else {
            f610e.m783b(str);
        }
    }

    public static void finishLevel(String str) {
        if (m765a(str)) {
            bl.m340e(f606a);
        } else if (str.length() > 64) {
            bl.m340e(f607b);
        } else {
            f610e.m785c(str);
        }
    }

    public static void failLevel(String str) {
        if (m765a(str)) {
            bl.m340e(f606a);
        } else if (str.length() > 64) {
            bl.m340e(f607b);
        } else {
            f610e.m786d(str);
        }
    }

    public static void pay(double d, double d2, int i) {
        if (i <= 0 || i >= 100) {
            bl.m340e(f609d);
        } else if (d < 0.0d || d2 < 0.0d) {
            bl.m340e(f608c);
        } else {
            f610e.m773a(d, d2, i);
        }
    }

    public static void pay(double d, String str, int i, double d2, int i2) {
        if (i2 <= 0 || i2 >= 100) {
            bl.m340e(f609d);
        } else if (d < 0.0d || i < 0 || d2 < 0.0d) {
            bl.m340e(f608c);
        } else if (m765a(str)) {
            bl.m340e(f606a);
        } else {
            f610e.m776a(d, str, i, d2, i2);
        }
    }

    public static void exchange(double d, String str, double d2, int i, String str2) {
        if (d < 0.0d || d2 < 0.0d) {
            bl.m340e(f608c);
        } else if (i <= 0 || i >= 100) {
            bl.m340e(f609d);
        } else {
            f610e.m775a(d, str, d2, i, str2);
        }
    }

    public static void buy(String str, int i, double d) {
        if (m765a(str)) {
            bl.m340e(f606a);
        } else if (i < 0 || d < 0.0d) {
            bl.m340e(f608c);
        } else {
            f610e.m779a(str, i, d);
        }
    }

    public static void use(String str, int i, double d) {
        if (m765a(str)) {
            bl.m340e(f606a);
        } else if (i < 0 || d < 0.0d) {
            bl.m340e(f608c);
        } else {
            f610e.m784b(str, i, d);
        }
    }

    public static void bonus(double d, int i) {
        if (d < 0.0d) {
            bl.m340e(f608c);
        } else if (i <= 0 || i >= 100) {
            bl.m340e(f609d);
        } else {
            f610e.m774a(d, i);
        }
    }

    public static void bonus(String str, int i, double d, int i2) {
        if (m765a(str)) {
            bl.m340e(f606a);
        } else if (i < 0 || d < 0.0d) {
            bl.m340e(f608c);
        } else if (i2 <= 0 || i2 >= 100) {
            bl.m340e(f609d);
        } else {
            f610e.m780a(str, i, d, i2);
        }
    }

    private static boolean m765a(String str) {
        if (str != null && str.trim().length() > 0) {
            return false;
        }
        return true;
    }

    public static void onEvent(String str, String str2) {
        MobclickAgent.onEvent(f611f, str, str2);
    }

    public static void onSocialEvent(Context context, String str, UMPlatformData... uMPlatformDataArr) {
        if (context == null) {
            bl.m340e("context is null in onShareEvent");
            return;
        }
        C0314e.f122e = "4";
        UMSocialService.share(context, str, uMPlatformDataArr);
    }

    public static void onSocialEvent(Context context, UMPlatformData... uMPlatformDataArr) {
        if (context == null) {
            bl.m340e("context is null in onShareEvent");
            return;
        }
        C0314e.f122e = "4";
        UMSocialService.share(context, uMPlatformDataArr);
    }
}
