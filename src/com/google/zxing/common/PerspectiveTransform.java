...
public final class PerspectiveTransform {
    ...
    public static PerspectiveTransform squareToQuadrilateral(float x0, float y0, float x1, float y1, float x2, float y2, float x3, float y3) {
        ...
        if (dx3 == 0.0f && dy3 == 0.0f) {
            return new PerspectiveTransform(x1 - x0, x2 - x1, x0, y1 - y0, y2 - y1, y0, 0.0f, 0.0f, DefaultRetryPolicy.DEFAULT_BACKOFF_MULT);
        }
        float dx1 = x1 - x2;
        float dx2 = x3 - x2;
        float dy1 = y1 - y2;
        ...
        return new PerspectiveTransform((x1 - x0) + (a13 * x1), (x3 - x0) + (a23 * x3), x0, (a13 * y1) + (y1 - y0), (a23 * y3) + (y3 - y0), y0, a13, a23, DefaultRetryPolicy.DEFAULT_BACKOFF_MULT);
    }
