...
public final class HighLevelEncoder {
    ...
    static int lookAheadTest(CharSequence msg, int startpos, int currentMode) {
        ...
        int minCount;
        if (currentMode == 0) {
            charCounts = new float[]{0.0f, DefaultRetryPolicy.DEFAULT_BACKOFF_MULT, DefaultRetryPolicy.DEFAULT_BACKOFF_MULT, DefaultRetryPolicy.DEFAULT_BACKOFF_MULT, DefaultRetryPolicy.DEFAULT_BACKOFF_MULT, 1.25f};
        } else {
            charCounts = new float[]{DefaultRetryPolicy.DEFAULT_BACKOFF_MULT, 2.0f, 2.0f, 2.0f, 2.0f, 2.25f};
        ...
        while (startpos + charsProcessed != msg.length()) {
            ...
            if (isDigit(c)) {
                ...
                charCounts[0] = (float) ((int) Math.ceil((double) charCounts[0]));
                charCounts[0] = charCounts[0] + DefaultRetryPolicy.DEFAULT_BACKOFF_MULT;
            }
            if (isNativeC40(c)) {
            ...
            if (isSpecialB256(c)) {
                ...
                charCounts[5] = charCounts[5] + DefaultRetryPolicy.DEFAULT_BACKOFF_MULT;
            }
            if (charsProcessed >= 4) {
