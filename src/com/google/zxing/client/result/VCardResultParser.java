...
public final class VCardResultParser extends ResultParser {
            ...
            if (i < 0) {
                ...
                if (quotedPrintable) {
                    ...
                    if (parseFieldDivider) {
                        element = UNESCAPED_SEMICOLONS.matcher(element).replaceAll(ShellUtils.COMMAND_LINE_END).trim();
                    }
                } else {
                    if (parseFieldDivider) {
                        element = UNESCAPED_SEMICOLONS.matcher(element).replaceAll(ShellUtils.COMMAND_LINE_END).trim();
                    }
                    element = VCARD_ESCAPES.matcher(NEWLINE_ESCAPE.matcher(CR_LF_SPACE_TAB.matcher(element).replaceAll("")).replaceAll(ShellUtils.COMMAND_LINE_END)).replaceAll("$1");
                }
                if (metadata == null) {
    ...
    private static void maybeAppendFragment(ByteArrayOutputStream fragmentBuffer, String charset, StringBuilder result) {
        if (fragmentBuffer.size() > 0) {
            ...
            if (charset == null) {
                fragment = new String(fragmentBytes, Charset.forName(Key.STRING_CHARSET_NAME));
            } else {
                try {
