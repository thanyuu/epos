...
public class Detector {
    ...
    protected final DetectorResult processFinderPatternInfo(FinderPatternInfo info) throws NotFoundException, FormatException {
        ...
        if (moduleSize < DefaultRetryPolicy.DEFAULT_BACKOFF_MULT) {
        ...
        if (provisionalVersion.getAlignmentPatternCenters().length > 0) {
            float correctionToTopLeft = DefaultRetryPolicy.DEFAULT_BACKOFF_MULT - (3.0f / ((float) modulesBetweenFPCenters));
    ...
    private float sizeOfBlackWhiteBlackRunBothWays(int fromX, int fromY, int toX, int toY) {
        ...
        return (result + sizeOfBlackWhiteBlackRun(fromX, fromY, (int) (((float) fromX) + (((float) (otherToX - fromX)) * scale)), otherToY)) - DefaultRetryPolicy.DEFAULT_BACKOFF_MULT;
    }
