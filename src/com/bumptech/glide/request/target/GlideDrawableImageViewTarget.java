...
public class GlideDrawableImageViewTarget extends ImageViewTarget<GlideDrawable> {
    ...
    public void onResourceReady(GlideDrawable resource, GlideAnimation<? super GlideDrawable> animation) {
        if (!resource.isAnimated()) {
            ...
            if (Math.abs((((float) ((ImageView) this.view).getWidth()) / ((float) ((ImageView) this.view).getHeight())) - DefaultRetryPolicy.DEFAULT_BACKOFF_MULT) <= SQUARE_RATIO_MARGIN && Math.abs(drawableRatio - DefaultRetryPolicy.DEFAULT_BACKOFF_MULT) <= SQUARE_RATIO_MARGIN) {
